...
----------------------------------------------
Prati dve pretrage na KupujemProdajem (SSD i RAM), filtrira po ceni, lokaciji
(Beograd) i kljucnoj reci u naslovu, i za svaki NOVI oglas salje poruku na
odgovarajuci Discord kanal preko webhook-a.

Ova verzija je napravljena da se pokrene JEDNOM i ugasi (ne radi u petlji) -
GitHub Actions je taj koji je pokrece iznova po rasporedu
...
NAPOMENA: ova verzija cita stranicu kategorije direktno (bez KP-ove ugradjene
pretrage), jer se pokazalo da kombinacija "keywords + so=1" parametara ume
da vrati pogresne/nepovezane rezultate. Filtriranje po reci (npr. "nvme")
radi se ovde u kodu, ne na samom sajtu.
"""
...
def parse_price_to_din(text):
    ...
    Vraca None ako cena nije prepoznata (npr. 'Kontakt' ili 'Kupujem')."""
...
def fetch_ads(search):
    """Ucitava prvu stranicu oglasa direktno iz kategorije na KupujemProdajem
    (stabilnije nego ugradjena pretraga sa keywords+so parametrima)."""
    url = (f"https://www.kupujemprodajem.com/{search['category_path']}"
           f"/grupa/{search['category_id']}/{search['group_id']}/1")
    ...
    for a in soup.find_all("a", href=True):
        ...
        keyword_ok = search["keyword"].lower() in title.lower()
